...
if __name__ == '__main__':
    ...
    for timestamp in sorted_timestamps:
        ann_array = timeline[timestamp]  # annotation tensor (I, J, K)
        # Check if the number of worker nodes is at least 112
        if ann_array.shape[2] != 112:
            print(f"Timestamp {timestamp} skipped: only {ann_array.shape[2]} worker nodes (<112).")
            continue
        
        print(f"Timestamp {timestamp}: Number of worker nodes = {ann_array.shape[2]}")
        
        preds = dawid_skene(ann_array, max_iter=1000, tol=0.001, smoothing_method="Laplace", coeff=0.01)
    ...
    np.save("sim_labels.npy", sim_labels)
